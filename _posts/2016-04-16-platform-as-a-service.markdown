...
## Team skills
When I started developing in the 90's, we were focused on deep infrastructural expertise. Databases, Operating systems, Dynamic loading, multi-threading  were my preferred topics. But today when discussing memory management strategies to my developer team, I feel like a dinosaur. 

Today my focus in hiring is to find full stack Web developers (worth to discuss hiring methodology in a future post, isn't it?) that have strong technology learning skills new and a genuine connection to the business user stories and not anymore developers expert shared libraries loading algorithms. This is a great evolution since we're now focusing more on business value and less on technical infrastructures.

So today, when it comes to setting up production servers, to configure databases, to ensure full redundancy, to deal properly with virtual machines, to setup disk volumes and filesystems, virtual memory, to manage access to static and dynamic files, we would need senior IT resources to work with a reliable hosting partner we would select. Then you would expect that each deployment will be a procedure that will require half day at best and one week at worst. 
...
Heroku is a exposing as a git repository interface so deploying to production is as simple as
...
There is much more to the [heroku command line](https://devcenter.heroku.com/articles/using-the-cli){:target="_blank"}, and in general the [heroku documentation](https://devcenter.heroku.com/categories/reference){:target="_blank"} is very comprehensive and developer friendly. 

## Multiple environments App reviews, alpha , beta and production
With Heroku we cloned our application to have 3 systems: alpha, beta and production. Every feature, developed in a [Git pull requests](https://help.github.com/articles/using-pull-requests/){:target="_blank"} is immediately testable via [Heroku Review Apps](https://devcenter.heroku.com/articles/github-integration-review-apps){:target="_blank"}, and once validated and code reviewed, it is pushed to our alpha system where all the product stake holders can validate and demo them on realistic data. Upon succesful review of the alpha system, we decide to expose the new features to our beta users that are using the system in real life conditions. After analyzing the feedback from our beta users and making the required corrections, we are ready to move to production, so we validate thorougly our release candidate (QA worth a separated post) and finalized the release, we push it to production, usually on a Sunday so our customers, so we do have all the required time create a clone system with the old version for reference, to face any unexpected data migrations issues, and to validate the new version to ensure all customers will be pleased on Monday morning to find a new release. 
...
<div style="text-align:center" markdown="1">
![Heroku pipeline](/assets/git-flow.png)
<br>
</div>
...
<div style="text-align:center" markdown="1">
![Github pull request automatically deployed in App Review](/assets/pullrequest.png)
<br>
</div>
